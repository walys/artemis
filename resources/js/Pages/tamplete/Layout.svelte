<script>
    ...
    export let apiUrl = $page.props.app?.apiUrl;
    export let appUserData = {};
    export let title;
    export let subtitle;
    let menuOpen = false;
    let users = {};
    let endpoint = apiUrl + "/autenticated";
    ...
    function initializeMenu() {
        ...
        console.log('Tentando inicializar menu...');
        console.log('Menu disponível:', typeof window.Menu);
        console.log('Helpers disponível:', typeof window.Helpers);
        
        if (typeof window.Menu !== 'undefined') {
            try {
                const layoutMenuEl = document.querySelector('#layout-menu');
                if (layoutMenuEl) {
                    console.log('Elemento do menu encontrado:', layoutMenuEl);
                    
                    const menu = new window.Menu(layoutMenuEl, {
                    ...
                    console.error('Elemento #layout-menu não encontrado');
                }
            } catch (error) {
                console.error('Erro ao inicializar menu:', error);
            }
        } else {
            console.error('Classe Menu não disponível');
        }
    }

    function initializeMenuTogglers() {
        if (togglersInitialized) {
            console.log('Menu togglers já inicializados');
            return;
        }
        
        console.log('Inicializando menu togglers...');
        
        const menuTogglers = document.querySelectorAll('.layout-menu-toggle');
        
        menuTogglers.forEach(toggler => {
            // Clonar o elemento para remover event listeners anteriores
            ...
            newToggler.addEventListener('click', function(event) {
                ...
                console.log('Menu toggle clicado - Listener direto');
                
                if (window.Helpers && typeof window.Helpers.toggleCollapsed === 'function') {
                    console.log('Usando Helpers.toggleCollapsed');
                    window.Helpers.toggleCollapsed();
                } else {
                    console.warn('Helpers.toggleCollapsed não disponível - usando fallback');
                    toggleMenuManual();
                }
            });
        });
        
        togglersInitialized = true;
        console.log('Menu togglers inicializados com sucesso');
    }

    // Fallback manual para toggle do menu
        ...
        <div class="layout-container">
            ...
            <aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme" style="touch-action: none; user-select: none;">
                ...
                <!-- <div class="app-brand demo ">
                    ...
                    <a on:click={toggleMenu} href="#" class="layout-menu-toggle menu-link text-large ms-auto">
